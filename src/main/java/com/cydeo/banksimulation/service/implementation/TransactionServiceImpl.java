...
public class TransactionServiceImpl implements TransactionService {
    ...
    private void validateAccounts(Account sender, Account receiver) {
        if(sender==null || receiver==null) {
            throw new BadRequestException("Sender or Receiver cannot be empty");
        }
    ...
    private void checkAccountOwnerShip(Account sender, Account receiver) {
        if ((sender.getAccountType().equals(AccountType.SAVINGS)|| receiver.getAccountType().equals(AccountType.SAVINGS))
            ...
            throw  new AccountOwnerShipException("When one of the account type is SAVINGS, User sender and receiver have to be same person");
        }
